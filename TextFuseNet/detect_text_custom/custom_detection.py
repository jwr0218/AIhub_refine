...
def make_image_crop(img_dir,prediction,polygons):
    ...
    for cnt, contour in enumerate(bboxes):
        ...
        try:
            ...
            cv2.imwrite('/workspace/TextDetection/TextFuseNet/detect_text_custom/new_cropped_box/'+img_name.split('.')[0]+"_{}_bbox.png".format(cnt),img_and)
        except:
...
if __name__ == "__main__":
    ...
    for idx , i in enumerate(os.listdir(test_images_path)):
        ...
        if idx % 1000 == 0 :
            print('현재 진행사항 {}/49252: '.format(idx))
        #print(i)
        img_name = os.path.basename(i)
        img = cv2.imread(test_images_path+i)
        img_save_path = output_path + img_name.split('.')[0] + '.jpg'
        
        test_labels_path = test_images_path.replace('images','labels') + img_name.split('.')[0] + '.json'
        #print(test_labels_path)


        with open(test_labels_path, "r") as json_file:
            json_dict = json.load(json_file)
            for data in json_dict['segments']:
                
                if data['type'] == '수식/텍스트' or data['type'] == '텍스트/수식':
                    # 여기서 함수 시자자자자자자자각    
                    torch.cuda.empty_cache()

                    # ============================================== 도형 부분만 남도록 하는 이미지 전처리 ========================
                    color = (255, 255, 255)

                    stencil = np.zeros(img.shape).astype(np.dtype("uint8"))
                    contour_ = np.array(data['box'],dtype = np.int32)
                    cv2.fillConvexPoly(stencil, contour_, color)
                    img= cv2.bitwise_and(stencil, img)
                    ...
                    tmp = check_intersection(img,test_images_path+i,prediction,polygons,figurebox)
                    if tmp != None:
                        #cv2.imwrite("/workspace/TextDetection/TextFuseNet/detect_text_custom/boxes/"+i,img)
                        
                        s = tmp[0]
                        dic = tmp[1]
                        dic['image_name'] = img_name.split('.')[0]  
                        if dic['intersection'] > 1 :
                            print(s)
                            print(dic) 
                            #sys.exit()
                        img_count +=1
                        #file.write(s)
                        #cv2.imwrite('/workspace/TextDetection/TextFuseNet/detect_text_custom/new_cropped_box/'+img_name,img)
                        make_image_crop(test_images_path+i,prediction,polygons)

                        pd_dic = pd.DataFrame.from_records([dic])
                        dataset = pd.concat([dataset,pd_dic])

    dataset.to_csv('/workspace/TextDetection/TextFuseNet/detect_text_custom/ratio_csv.csv')
    #file.close()
